    ...
    render() {
        return (
            <Layout style={{ background: '#fff', height: "100%" }}>
                ...
                <Content>
                    <Dialog title={"添加【" + this.state.selectTreeName + "】"} visible={this.state.dialog}
                        handleOk={this.handleOk.bind(this)}
                        handleCancel={this.handleCancel.bind(this)}>
                        <WrappedNormalAddMenuFrom viewCode={this.state.viewcode} />
                    </Dialog>
                    <div style={{ padding: 5 }}>
                        <Button disabled={this.state.addButton} type="primary" onClick={this.addMenu.bind(this)}>添加{this.state.selectTreeName == "" ? "菜单" : this.state.selectTreeName}</Button>
                        <Table
                            bordered
                            columns={columns}
                            rowKey={record => {
